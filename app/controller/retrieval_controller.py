...
Out of scope for this phase (later phases): multi-intent decomposition (the RETRIEVE query here is
always the whole buffer, treated as one undecomposed unit), RRF/reranking, session-answer
refinement, grounding, answer generation. `check_suppression`'s ambiguous-match branch is
deterministic-only here (no LLM call) — see app.controller.suppression.
"""
...
async def on_chunk(
    ...
    *,
    ...
    return await _retrieve(session, chunk, trace_id, retriever, sink, trigger)
